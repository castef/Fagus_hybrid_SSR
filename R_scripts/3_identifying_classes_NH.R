...
# renaming classes
indiv_class$NH_class_t80 <- factor(indiv_class$NH_class_t80, levels = c("Pure1", "Pure2", "F1", "F2", "BC1", "BC2"),
                           labels = c("Pure sylvatica", "Pure orientalis", "F1", "F2","BC sylvatica", "BC orientalis"))

# check how many individuals per class
table(indiv_class$generation, indiv_class$NH_class_t80)

# save file
write.csv(indiv_class, "NewHybrids/Allenwiller_indiv_info_final.csv")

## reduced dataset
# merging classification results with tree info file
indiv_info <- read.csv("Allenwiller_indiv_info_withcoord_subset.csv")
indiv_class <- merge(indiv_info, nh_res[,c("IndivName", "NH_max_class", "NH_class_t80")], by.x = "SampleID", by.y = "IndivName", all.x = TRUE, all.y = TRUE)
...
# save file
write.csv(indiv_class, "NewHybrids/Allenwiller_indiv_info_class_subset.csv")
...
indiv_class[which(indiv_class$NH_class_t80_2 == "BC orientalis"),]
nh_res[which(nh_res$NH_class_t80 == "BC2"),]

# correct and classify as unassigned
indiv_class$NH_class_t80_corr = indiv_class$NH_class_t80_2
indiv_class[which(indiv_class$NH_class_t80_corr == "BC orientalis"),"NH_class_t80_corr"] = "unassigned"

table(indiv_class$generation, indiv_class$NH_class_t80_corr)

# claculating percentages of each class per geneariont
perc_wal <- indiv_class %>%
  group_by(generation, NH_class_t80_corr) %>%
  summarise(count = n()) %>%
  group_by(generation) %>%
...
colnames(indiv_class)[c(11,12)] <- c("NH_class_t80", "NH_class_t80_corrected")
...
perc_al <- indiv_class %>%
  group_by(generation, NH_class_t80_2) %>%
  summarise(count = n()) %>%
  group_by(generation) %>%
  mutate(total = sum(count),
         percent = 100 * count / total) %>%
...
obs_proportion <- obs %>%
  group_by(stand, generation, NH_class_t80) %>%
  summarize(genot_n = n(), .groups = "drop") %>% 
  group_by(stand, generation) %>% # total number of individuals per stand and stage_group
...
obs_proportion <- obs %>%
  group_by(stand, generation, NH_class_t80) %>%
  summarize(genot_n = n(), .groups = "drop") %>% 
  group_by(stand, generation) %>% # total number of individuals per stand and age group
  mutate(stage_tot = sum(genot_n),
         genot_prop = genot_n / stage_tot) %>% # proportion of each genotype within the stage group
  ungroup()
obs_proportion
