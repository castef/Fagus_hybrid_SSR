...
ColourVector1 <- c("N_P1" = "#FDE725FF", "N_P2" = "#482173FF", "N_F1" = "#25858EFF", "N_F2" = "#85D54AFF", "N_BC1" = "orange", "N_BC2" = "#9C99C8", "N_unassigned" = "black")
ColourVector2 = c("Pure1" = "#FDE725FF", "Pure2" = "#482173FF", "F1" = "#25858EFF", "F2" = "#85D54AFF", "BC1" = "orange","BC2" ="#9C99C8", "unassigned" = "black")
...
## with unassigned
ColourVector3 <- c("Pure sylvatica" = "#FDE725FF",
...
## with unassigned
ColourVector4 <- c("Pure sylvatica" = "#FDE725FF",
                   "Pure hohenackeriana" = "#482173FF",
                   "F1" = "#25858EFF",
                   "F2" = "#85D54AFF",
                   "BC sylvatica" = "orange",
...
waldi_ss <- ggplot() +
  ...
  geom_point(data = obs, aes(x = x, y = y),size = 0.3, fill ="black", color = "black", stroke = 1, shape = 21) +
...
allen_ss <- ggplot() +
  ...
  geom_point(data = obs, aes(x = x, y = y), fill ="black", color = "black",size = 0.3, alpha = 0.8,stroke = 1, shape = 21) +
...
## get the geopolitical or terrain map
...
final_plot <- (allen_ss | base_map | waldi_ss ) +
  plot_layout(guides = "collect", widths = c(0.94, 0.6,1)) &  
...
# -------- Figure 2. NH results plots and maps ------
        ...
        generation = read.csv(paste0(path, "NewHybrids/Results/Allenwiller/Allenwiller_coord_circumf_class80ppt.csv"))
        NH_melt = merge(NH_melt, generation[, 2:3], by = "IndivName")
        ...
        al_nh_plot = ggplot(NH_melt, aes(x = IndivName, y=as.numeric(CumProb), fill=PopProb2))+ # CumProb changed to numeric
          ...
          theme(axis.title.x = element_text(size = 13, vjust = -3), axis.text.x = element_blank(),
                axis.text.y = element_text(size = 7), axis.title.y = element_text(size = 13),
          ...
          guides(colour = "none",fill=guide_legend(nrow=7, title = "")) +
...
# add information of generation 
generation1 = read.csv(paste0(path, "NewHybrids/Results/Waldi/Waldi_coord_circumf_class80ppt.csv"))

NH_melt1 = merge(NH_melt1, generation1[, 2:3], by = "IndivName")
...
wal_nh_plot = ggplot(NH_melt1, aes(x = IndivName, y=as.numeric(CumProb), fill=PopProb2))+ # CumProb changed to numeric
  ...
  ggtitle("Wäldi")+
  guides(colour = "none",fill="none") +
  facet_wrap(~gen, drop = T, scales = "free", ncol = 1)
wal_nh_plot
...
## Allenwiller (RUN THIS OR THE SUBSET VERSION)  ## 
all_tree = fread(paste0(path, "Data_clean/NewHybrids/Allenwiller_indiv_info_class.csv"))
...
mt_cp <- all_cp %>%
  ...
  summarise(
    N_tot = n(),
    N_P1 = sum(NH_class_t80 == "Pure sylvatica", na.rm = TRUE),
    N_P2 = sum(NH_class_t80 == "Pure orientalis", na.rm = TRUE),
    N_F1 = sum(NH_class_t80 == "F1", na.rm = TRUE), 
    N_BC1 = sum(NH_class_t80 == "BC sylvatica", na.rm = TRUE),
    N_BC2 = sum(NH_class_t80 == "BC orientalis", na.rm = TRUE),
    N_F2 = sum(NH_class_t80 == "F2", na.rm = TRUE),
...
all_cp_grouped[which.min(all_cp_grouped$y), "x"] <- all_cp_grouped[which.min(all_cp_grouped$y), "x"] - 6

## convert orientalis --> hohenackeriana
all_tree$NH_class_t80 <- gsub("orientalis", "hohenackeriana", all_tree$NH_class_t80)
wal_tree$NH_class_t80_corrected <- gsub("orientalis", "hohenackeriana", wal_tree$NH_class_t80_corrected)

# allenwiller full version
all_map= ggplot()+
  ...
  new_scale("fill")+
  geom_scatterpie(data = all_cp_grouped, mapping = aes(x = x, y = y, r=8),colour="black",alpha = 0.8,
                  cols = c("N_P1", "N_P2", "N_F1", "N_F2" ,"N_BC1" ,"N_BC2"))+
  scale_fill_manual(values = ColourVector1,labels=c("Pure sylvatica", "Pure hohenackeriana", "F1","F2", "BC slvatica", "unassigned"))+
  geom_point(data=subset(all_tree,generation == "Adult"), mapping = aes(x=x,y=y,color=NH_class_t80),size=1.2)+
  scale_color_manual(values = ColourVector3, expand = c(0, 0))+
  theme_void()+ guides(fill = "none")+
  coord_sf(
    xlim = c(min(all_tree$x, na.rm = T)-5, max(all_tree$x, na.rm = T)+5),
    ylim = c(min(all_tree$y, na.rm = T)-5, max(all_tree$y, na.rm = T)+5),
    expand = FALSE
  )+
  theme( strip.background = element_blank()) + 
  guides(colour = "none",fill=guide_legend(nrow=7, title = "")) +
  annotation_scale(location = "bl", width_hint = 0.5, style = "ticks", line_col = "black", text_col = "black", line_width = 2, text_cex = 1)
all_map
...
all_map_subset= ggplot()+
  ...
  scale_fill_manual(values = ColourVector1,labels=c("Pure sylvatica", "Pure hohenackeriana", "F1","F2", "BC slvatica", "unassigned"))+
  geom_point(data=subset(all_tree,generation == "Adult"), mapping = aes(x=x,y=y,color=NH_class_t80),size=0.8)+
  scale_color_manual(values = ColourVector3, expand = c(0, 0))+
  theme_void()+ guides(fill = "none")+
  coord_sf(
...
wal_map = ggplot()+
  ...
  scale_fill_manual(values = c("N_P1" = "#F0E442", "N_P2" = "#482173FF", "N_F1" = "#25858EFF"), labels=c("Pure sylvatica", "Pure hohenackeriana", "F1","unassigned"))+
  geom_point(data=subset(wal_tree,generation == "Adult"),aes(x=x,y=y,color=NH_class_t80_corrected),size=0.8)+
...
gc()
          ...
          # full version 
          Fig_NH <- ggpubr::ggarrange(
            al_nh_plot, wal_nh_plot, all_map, wal_map,
            nrow = 2, ncol = 2,
            legend = "right",
            common.legend = TRUE,
            align = "v",
            heights = c(1, 2)  # increase space between the rows
          )  
          
          Fig_NH
          
          png(filename=paste0(path, "Manuscript/Figures/Figure_NH.png"), width =19000,height=15000, res=1300)
          plot(Fig_NH)
          dev.off()
...
Fig_NH_subset <- ggpubr::ggarrange(
  ...
  labels = c("A","","B",""),
  align = "hv",
  heights = c(1.5, 2)  # increase space between the rows
)  

Fig_NH_subset <- ggarrange(Fig_NH_subset, legend_no_unassigned, ncol = 2, widths = c(5,1))
Fig_NH_subset

png(filename=paste0(path, "Manuscript/Figures/Figure2_NH_subset.png"), 
...
ColourVector = c("#FDE725FF", "#482173FF", "#25858EFF", "#85D54AFF", "orange", "#9C99C8")
...
all_plot <- ggplot(subset(all_res,stand == "Allenwiller"), aes(x = PostProb, y = Value)) +
  ...
  coord_cartesian(ylim = c(0, 1)) + theme_bw(base_size = 7)+
        theme(panel.background = element_rect(fill = "white", colour = "black"), 
              ...
              strip.background = element_rect(colour = "white", fill = "white"), 
              plot.title = element_text(hjust = 0.5,vjust = 1, size = 8, face = "bold", margin = margin(b = 0)))+
  guides(fill = "none",color=guide_legend(ncol=1, title = ""))+
  ggtitle("Allenwiller")
all_plot
...
t_dat = fread("Data_clean/Phenology/T_GDD_CD_all_sites.csv")
...
all_dat <- fread("Data_clean/Phenology/Spring_phenology_all_sites_final.csv")
...
waldi_pheno <- ggplot(subset(summary,Measure !="CD"),aes(x = x_var, y = mean_stage, col = species, fill = species)) +
  ...
  ggh4x::facet_grid2(year ~ Measure, scales = "free_x",
             labeller = labeller(Measure = c(doy = "Day of Year (DOY)", Cum_GDD = "Growing Degree Days (GDD)")), switch = "x") +
  colSet + fillSet + theme_bw(base_size = 7) +
...
all_boxplot <- ggplot(data = subset(all_dat,site == "Allenwiller"), aes(x = as.factor(doy), y = average.stage, fill = species, group = species)) +
  ...
  geom_boxplot(alpha = 0.8, col = "black", linewidth =0.3) +
  ...
  stat_compare_means(
    ...
    aes(group = species),
    label = "p.signif",
    label.y = wilcox_df$label_y,     # same as other plot
    size = 2.5
  )+
  theme_bw(base_size = 7) +
...
# -------- Figure S4. NMpi2 results - selection gradients ---------
...
sel_grad$labels2 = factor(sel_grad$Sel_gradient,
                          ...
                          labels = c("Tree size", "Competition", "Species effect", "assortative mating"))

sel_grad_subset <- subset(sel_grad, labels2 != "assortative mating")

sel_grad_plot = ggplot(sel_grad_subset, 
                       mapping = aes(x = as.numeric(Effect), y = labels2)) + 
  geom_point(position = position_dodge(width = 0.5), size = 1) +
  geom_errorbarh(aes(xmin = as.numeric(Effect) - SE*1.96, xmax = as.numeric(Effect) + SE*1.96), 
                 height = 0.1, size = 0.6, position = position_dodge(width = 0.1)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black", alpha = 0.6) +
  facet_grid(Fem_male ~ Site) +
  ...
  labs(y = "Phenotypic character", x = "Standardized effect on reproductive success") +
  xlim(c(-1.9, 1.9)) +
...
path="C:/Users/stefanin/Dropbox/WSL_PhD/Projects/Hybridization/"
png(filename=paste0(path, "Manuscript/Figures/FigureS4_selectiongradients.png"), 
    width =4,height=3, res=600, units = "in")
plot(sel_grad_plot)
dev.off()

# -------- Figure S5. NMPi2 results - genealogies -----
...
### A:  offspring genotype for each type of genealogy
combined <- bind_rows(
...
panel_A <- ggplot(combined, aes(x = Prog_NH, fill = group)) +
  geom_bar(position = "fill", col = "black", width = box_width, linewidth = line_width) +
  scale_fill_manual(values = c("white", "grey", "black")) +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(title = "",x = "Offspring genotype", y = "Percentage", fill = "N. of assigned\nparents") +
  ...
  theme(panel.background = element_rect(fill = "white", colour = "black"), 
        ...
        legend.key = element_rect(colour = "white"),
        legend.text = element_text(size = 3),           # Text size in legend
        legend.title = element_text(size = 4),          # Legend title size
        legend.key.size = unit(0.2, "cm"),              # Size of legend keys (squares)
        legend.key.width = unit(0.2, "cm"),             # Width of legend keys
        legend.key.height = unit(0.2, "cm"),            # Height of legend keys
...
### B :genotpye of single parent of each offspring
one_parent$Prog_NH <- factor(one_parent$Prog_NH, levels = names(ColourVector3))

panel_B <- ggplot(one_parent, aes(x = Prog_NH, fill = Mo1_NH)) +
  geom_bar(position = "fill", col = "black", width = box_width, linewidth = line_width) +
  scale_fill_manual(values = ColourVector3) +
  scale_y_continuous(labels = scales::percent_format()) +
...
### C: spatial relationship of F1 with their parents (both assigned)
...
panel_C <- ggplot(dist_compare, aes(x = Parent_Type, y = Distance, fill = Parent_Type, colour = Parent_Type)) +
  geom_boxplot(alpha = 0.8, col = "black",width = box_width, linewidth = line_width,outlier.size = 0.5) + 
  guides(fill = "none")+
  #geom_point()+
  scale_fill_manual(values = ColourVector3)+  
...
### D:distance of the single parent from the offspring
f1_one <- f1_one %>%
  mutate(
...
panel_D <- ggplot(f1_one, aes(x = Parent_NH, y = dist_to_parent, fill = Parent_NH, col = Parent_NH)) +
  geom_boxplot(alpha = 0.8, col = "black", width = box_width, linewidth = line_width,outlier.size = 0.5) + 
  guides(fill = "none")+
  #geom_point()+
  scale_fill_manual(values = ColourVector3)+  
...
## combine plots
nmpi_genealogies2 <- ggpubr::ggarrange(panel_A,panel_B,
                                       panel_C,panel_D,
...
nmpi_genealogies3 <- ggpubr::ggarrange(nmpi_genealogies2,
                                       legend_with_unassigned,
                                       nrow = 1, ncol = 2,widths = c(5,1), align = "h") 
nmpi_genealogies3

path="C:/Users/stefanin/Dropbox/WSL_PhD/Projects/Hybridization/"
png(filename=paste0(path, "Manuscript/Figures/FigureS5_genealogies.png"), 
    width =7, height=4.8, units = "in", res=600)
plot(nmpi_genealogies3)
dev.off()


# -------- Figure S6. NEMO starting scenario and demographic parameters -------
...
all_grid <- ggplot(grid_al) +
  geom_spatvector(fill = "grey90")+theme_void()+
...
wal_grid <- ggplot(grid_wal) +
  ...
  scale_color_manual(values= ColourVector3) + 
  guides(fill = "none",color=guide_legend(nrow=2, title = ""))+
  theme( plot.title = element_text(hjust = 0.5,vjust = 1, size = 8, face = "bold", margin = margin(b = 0)), 
        legend.position = "bottom", 
        legend.background = element_rect("white", "white"),
        legend.key = element_rect(colour = "white"),
        legend.text = element_text(size = 5),      # Smaller legend text
        ...
        legend.spacing.y = unit(0.05, "cm"),        # Less spacing
        legend.key.spacing.y = unit(0.05, 'cm'), 
        legend.margin = margin(0, 2, 12, 2),
  )+
  ggtitle("Wäldi") + labs(col = "")
wal_grid
...
combined_fem_data <- readRDS( "Data_clean/NEMO/Demographic_test_results.RDS")
...
pop_trend_w <- ggplot() +
  geom_line(data = subset(summary_data_agg, stand == "Waldi"), aes(x = generation, y = N_agg_stage_scaled,  group = interaction(agg_stage, replicate),color = agg_stage,linetype = data_source), linewidth= 0.5) +
...
pop_trend_a <- ggplot() +
  geom_line(data = subset(summary_data_agg, stand == "Allenwiller"), aes(x = generation, y = N_agg_stage_scaled, group = interaction(agg_stage, replicate),color = agg_stage,linetype = data_source), linewidth= 0.5) +
...
Fig_nemodemographic <- ggpubr::ggarrange(
  ...
  nrow = 1, ncol = 2,widths = c(1,3), heights = c(2,4),
  labels = c("A", "B"),
  align = "h"
)  
...
path="C:/Users/stefanin/Dropbox/WSL_PhD/Projects/Hybridization/"
png(filename=paste0(path, "Manuscript/Figures/FigureS6_nemodemographic.png"), 
    width =7,height=7, res=600, units = "in")
plot(Fig_nemodemographic)
dev.off()

# -------- Figure 4 and S7. NEMO HI patterns 2020 and 2120 ----
...
grid_off_2020 <- readRDS("NEMO/HI_data_offspring_2020.RDS")
grid_off_2120 <- readRDS("NEMO/HI_data_offspring_2120.RDS")
...
grid_adults_2020 <- grid_adults_2020 %>% left_join(sim_HI_2020_summary[which(sim_HI_2020_summary$stage == 3), c("stand", "scenario2", "hybridization_rate")],  by = c("stand", "scenario2"))
grid_adults_2120 <- grid_adults_2120 %>% left_join(sim_HI_2120_summary[which(sim_HI_2120_summary$stage == 3), c("stand", "scenario2", "hybridization_rate")],  by = c("stand", "scenario2"))
grid_off_2020 <- grid_off_2020 %>% left_join(sim_HI_2020_summary[which(sim_HI_2020_summary$stage == 1), c("stand", "scenario2", "hybridization_rate")],  by = c("stand", "scenario2"))
grid_off_2120 <- grid_off_2120 %>% left_join(sim_HI_2120_summary[which(sim_HI_2120_summary$stage == 1), c("stand", "scenario2", "hybridization_rate")],  by = c("stand", "scenario2"))
...
plot_HI <- function(data_subset, title, sampling_data = NULL) {
  ...
  p <- ggplot(data_subset) +
    ...
    guides(alpha = "none") +
    geom_text(aes(x = Inf, y = -Inf, label = paste0(round(hybridization_rate, 2))),
              hjust = 1.1, vjust = -0.5, size = 1.8, inherit.aes = FALSE)+
...
adults_2020 <- ggpubr::ggarrange(
  al_adults_30+ theme(axis.title.x = element_blank(), strip.text.x =element_blank(), legend.position ="none"),
  wal_adults_30+ theme(axis.title.x = element_blank(), axis.title.y = element_blank(),strip.text.x =element_blank(),  strip.text.y.left = element_blank(), legend.position ="none"),
  nrow = 1, ncol = 2,
  labels = c("Allenwiller", "Wäldi"),
  label.x = 0.3,  label.y = 1,
...
# offspring 2020 (only Ar = 30)
wal_off_30 <- plot_HI(subset(grid_off_2020, stand == "Waldi" & Ar == 30),"",sampling_data = sampling_all %>% filter(stand == "Waldi"))
al_off_30 <- plot_HI(subset(grid_off_2020, stand == "Allenwiller" & Ar == 30), "",sampling_data = sampling_all %>% filter(stand == "Allenwiller"))

offspring_2020 <- ggpubr::ggarrange(
  al_off_30+ theme( legend.position ="none"), 
  wal_off_30 + theme( strip.text.y.left = element_blank(),axis.title.y = element_blank(), legend.position ="none"),
  nrow = 1, ncol = 2,
  align = "hv"
)  
...
# merge Figure 2020
Figure_HI_2020 <- ggpubr::ggarrange(
  ggpubr::ggarrange(
    adults_2020+ theme(axis.title.x = element_blank(), strip.text.x =element_blank(),legend.position ="none"), 
    offspring_2020 + theme(legend.position ="none"), nrow = 2, ncol = 1, labels = c("A", "B"),label.x = 0.05,  label.y = 1.02,
    font.label = list(size = 12, face = "bold"), align = "hv"),
  legend_HI,
...
path="C:/Users/stefanin/Dropbox/WSL_PhD/Projects/Hybridization/"
png(filename=paste0(path, "Manuscript/Figures/Figure4_HI_2020.png"), 
    width =7, height=6, units = "in", res=600)
plot(Figure_HI_2020)
dev.off()


####### future HI distributions 
...
Figure_adults_sim_HI_grids_2120 <-ggpubr::ggarrange(Figure_adults_sim_HI_grids_2120,legend_HI, ncol = 2, nrow = 1, widths = c(5,1))
Figure_adults_sim_HI_grids_2120

path="C:/Users/stefanin/Dropbox/WSL_PhD/Projects/Hybridization/"
png(filename=paste0(path, "Manuscript/Figures/FigureS7A_adults_sim_HI_grids_2120.png"), 
    width =7, height=6, units = "in", res=600)
...
wal_off_30 <- plot_HI(subset(grid_off_2120, stand == "Waldi" & Ar == 30),"Ar = 30",sampling_data = sampling_all %>% filter(stand == "Waldi"))
wal_off_50 <- plot_HI(subset(grid_off_2120, stand == "Waldi" & Ar == 50), "Ar = 50",sampling_data = sampling_all %>% filter(stand == "Waldi"))
al_off_30 <- plot_HI(subset(grid_off_2120, stand == "Allenwiller" & Ar == 30), "Ar = 30",sampling_data = sampling_all %>% filter(stand == "Allenwiller"))
al_off_50 <- plot_HI(subset(grid_off_2120, stand == "Allenwiller" & Ar == 50), "Ar = 50",sampling_data = sampling_all %>% filter(stand == "Allenwiller"))
...
Figure_off_sim_HI_grids_2120 <-ggpubr::ggarrange(Figure_off_sim_HI_grids_2120,legend_HI, ncol = 2, nrow = 1, widths = c(5,1))
Figure_off_sim_HI_grids_2120

path="C:/Users/stefanin/Dropbox/WSL_PhD/Projects/Hybridization/"
png(filename=paste0(path, "Manuscript/Figures/FigureS7B_offs_sim_HI_grids_2120.png"), 
    width =7, height=6, units = "in", res=600)
plot(Figure_off_sim_HI_grids_2120)
dev.off()

# -------- Figure S8A_B. difference P_hi(SIM - OBS) in the circle plots -----------
...
# -------- Figure S8C. plot SSE circle plots ----------
...
path="C:/Users/stefanin/Dropbox/WSL_PhD/Projects/Hybridization/"
png(filename=paste0(path, "Manuscript/Figures/FigureS8C_SSE_grid.png"), 
    width =7.0866, height=7.0866, units = "in", res=600)
plot(SSE_plot)
dev.off()

# -------- Figure 5. plot HI best scenarios current future ------
setwd("C:/Users/stefanin/Dropbox/WSL_PhD/Projects/Hybridization/Data_clean/")
...
# -------- Figure S9. Figure Nemo SSE and Moran's I -----
# run script 10 first
