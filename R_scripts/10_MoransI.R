...
sampling_shp <- sampling_shp[sampling_shp$CirclePlot%in% wal_valid_cp,]
centroids <- st_centroid(sampling_shp)
coords <- st_coordinates(centroids)

# distance matrix coordinates circle plots
...
merged_obs <- sampling_shp %>%left_join(wal_cp_HI, by = c("CirclePlot" = "CirclePlotID"))
...
diag(inv_dist_matrix) <- 0 
# attach HI value to the shapefile
merged_obs <- sampling_shp %>%left_join(al_cp_HI, by = c("CirclePlotID" = "CirclePlotID"))
...
for (i in seq_along(stands)) {
  ...
  stand_HI_cp <- sim_HI_cp %>%
    filter(stand == stand_name, CirclePlotID %in% valid_cp)
  
  # Read the corresponding sampling shapefile
  sampling_shp <- st_read(sampling_file)
  colnames(sampling_shp)[2]<- "CirclePlotID"
  
  # filter the shapefile for the valid circle plots
  sampling_shp <- sampling_shp %>%
    filter(CirclePlotID %in% stand_HI_cp$CirclePlotID)
  
  # Calculate centroids and distance matrix
  centroids <- st_centroid(sampling_shp)
  coords <- st_coordinates(centroids)
  dist_matrix <- as.matrix(dist(coords))
  ...
  # Get unique combinations of scenario and replicate for the current stand
  combinations <- stand_HI_cp %>%
    distinct(stand, scenario2, replicate)
  
  # Calculate Moran's I for each combination
  for (j in seq_len(nrow(combinations))) {
    ...
    # Moran's I calculation with error handling
    moran_result <- tryCatch({
      Moran.I(combo_HI$prop_HI1, inv_dist_matrix)
    }, error = function(e) {
